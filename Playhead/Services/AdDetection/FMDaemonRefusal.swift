...
// transcript stops moving, which is exactly when a full-episode ad scan is
// worth running and is nearly all of an episode's life. The disposition is
// wrong either way — a daemon condition is not evidence about the job at any
// scope — but the cost is "this episode is disqualified against its FINAL
// transcript", not "forever, from the first failure".
//
...
enum FMDaemonRefusal: Sendable, Equatable, CaseIterable {
    ...
    static func shouldStopDraining(consecutiveRefusals: Int) -> Bool {
        FMDaemonThrottle.shouldStopDraining(consecutiveDaemonRefusals: consecutiveRefusals)
    }
}
