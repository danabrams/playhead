...
enum SourceEvidenceFamily: String, Sendable, Equatable, CaseIterable {
    ...
    static func `for`(_ source: EvidenceSourceType) -> SourceEvidenceFamily {
        switch source {
        case .lexical, .classifier, .lexicalAutoAd, .rhetoricalGrammar:
            ...
            // playhead-xsdz.12: the rhetorical act-sequence grammar is also a
            // text-derived signal (it classifies persuasion roles in the span's
            // transcript prose), so it shares the `.textual` family. Same
            // orthogonality guarantee: a `.rhetoricalGrammar` corroborator
            // cannot self-corroborate against the lexical channels it sits
            // alongside in the same family.
            return .textual
        ...
        case .fingerprint, .catalog, .crossEpisodeMemory, .crossShowSyndication:
            ...
            // playhead-xsdz.13: cross-show syndication is also a cross-library
            // reference-match signal (it compares the candidate's sponsor entity
            // against a stored aggregate of the user's own library), so it shares
            // the `.reference` family. Same orthogonality guarantee: a
            // `.crossShowSyndication` boost cannot self-corroborate against the
            // reference channels it sits alongside.
            return .reference
        case .fusedScore:
            return .model
        case .metadata:
