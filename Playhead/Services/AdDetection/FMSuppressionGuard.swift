...
struct FMSuppressionApplicator: Sendable {
    ...
    private func isStrongEvidence(_ entry: EvidenceLedgerEntry) -> Bool {
        ...
        case .lexicalAutoAd:
            ...
            return true
        case .classifier, .lexical, .acoustic, .musicBed, .breakAlignment, .audioForensics, .crossEpisodeMemory, .rhetoricalGrammar, .crossShowSyndication, .catalog, .fusedScore, .metadata, .audit, .operational:
            ...
            // playhead-xsdz.12: rhetoricalGrammar is a MODEST text-derived
            // corroborator (cap 0.20, no qualified track); it is a soft signal
            // that yields to FM noAds suppression like the lexical channels.
            return false
        }
    }
}
