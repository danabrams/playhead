// DebugEpisodeExportService.swift
// Non-UI service that consults `AnalysisStore` to assemble debug exports
// (per-episode and library-wide) for the Settings debug section.
//
// Why this service exists (playhead-fwvz):
// The export's *formatting* code is harmless from a module-boundary
// standpoint, but the *fetch* code touched `AnalysisStore` directly.
// `SurfaceStatusUILintTests.testNoSchedulerOrPersistenceTypesInUIViews`
// forbids any UI-layer file from referencing `AnalysisStore` (or other
// persistence/analysis types). Moving the fetch + format into a
// `Playhead/Services/Diagnostics/` service lets the SwiftUI Settings
// view stay on the `EpisodeSurfaceStatus`-only contract while still
// producing the same `DebugEpisodeExport` value for ShareLink.
//
// This file is the ONLY place these export pipelines reach into the
// store; the UI-side `DebugEpisodeExport` (Transferable) remains in the
// view layer because it is a SwiftUI-binding type used by ShareLink.

#if DEBUG

import Foundation

/// Non-UI helper namespace for building debug episode exports. The
/// SwiftUI Settings section calls `build(...)` / `buildLibraryExport(...)`
/// and feeds the resulting `DebugEpisodeExport` to a `ShareLink`.
enum DebugEpisodeExportService {

    /// Build an export for the current episode. Returns `nil` if the
    /// store fetch fails or no episode is loaded.
    static func build(
        episodeTitle: String,
        podcastTitle: String,
