...
**THE UNION DOES NOT SATURATE, and the `~47` above is roughly 8x low (measured 2026-08-13, playhead-o89d).** "Once the union saturates, `RED (N known / 0 new)` is the steady state" was the plan and the data refuses it. New names per accepted run, reconstructed from `runs_observed − seen_runs + 1`: **32, 41, 69, 73, 73, 81, 81, 83, 117**, then **+10 and +9** from the two full-plan logs of 2026-08-13 — still climbing ~9–10 per run after eleven observations, with no knee. Chao1 on the file's own failure-frequency spectrum (f1 = 51 names failed in exactly one run, f2 = 5 in exactly two) gives a **lower bound of 377** against **136 ever observed**; 390 tests carry a `.timeLimit(.minutes(1))`, which is where 377 lands. `~47` came from capture–recapture on two runs, and two runs cannot see a tail that is half singletons. So the recorded set is a **~35 % sample** of a population no name list can bound, and `RED (N known / ~10 new)` — not `/ 0 new` — is the honest steady state until something moves the population itself.

**Two corrections to how a red gate is read, from the same measurement.** First, **`passed after N seconds` is enqueue-to-completion, not test cost, and the 97 s triage rule reads it as though it were cost.** In a full plan **82–89 % of PASSING tests report over 60 s**, the median is ~150 s, and **291 tests carrying a 60 s time limit passed while reporting 151–198 s**. The clean witness is a test that prints its own `ContinuousClock` reading: `benchmark gate` reported **220.1 s** while its own clock read **105.5 s**. So "under 97 s" selects tests that failed EARLY in the run, not tests that failed for behavioural reasons. Second, **"asserts an absolute wall-clock budget" is the wrong predicate** — CPU-bound budgets are fine under full load (`evaluate() completes in <200ms`, `detectRefined … within budget` at 200 ms, and the 5 s `PerformanceSmokeTests` all PASS in the full plan). What starves is **scheduler-dependent waiting** measured on an absolute clock: a poll loop, an `AsyncStream` consumer, a cross-actor callback. 135 of the 136 recorded names pass **5/5** when the same 52 suites are run scoped (443 tests, 27.9 s).
