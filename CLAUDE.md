...
Method, so it can be re-run: every preserved log that is non-selective, started **more than 11,000 tests** and lost **fewer than 100 verdicts**, de-duplicated by `(start count, failure set)`, parsed by `gate_baseline.parse_run` itself rather than by a fresh regex. Re-run at R3 over **every** `.log` under `$TMPDIR/claude-501/-Users-dabrams-playhead/*/scratchpad`, `/private/tmp` and `.worktrees`: **39 qualifying logs, 38 after de-duplication, 2026-08-09 18:37 → 2026-08-13 12:41**. Union: **269 distinct failing tests**.

**R2 stated the same criteria and reported 28 runs / union 243, and its own census of logs was the incomplete half** — the defect it had just charged R1 with ("there are nineteen preserved logs that day, not four"), one level up. Four of the 38 live outside the session scratchpad that R2 walked (`/private/tmp/gate-baseline-refresh.log` and three under a second session's directory); the other six are unexplained by any variant of the stated filter that was tried. Prefer 38/269, and **name the roots you walked**, because "every preserved log" is a claim about a search, not about a set.
