...
**THE POPULATION IS BOUNDED AND THE OBSERVED UNION HAS VERY NEARLY ENUMERATED IT — measured directly at o89d R2, and two earlier versions of this paragraph claimed the opposite.** Both of those were estimates (a Chao1 "lower bound of 377", withdrawn below; then "the union does not saturate … there is no knee"). What follows is the direct measurement of the quantity they were estimating, so prefer it over either.

Method, so it can be re-run: every preserved log that is non-selective, started **more than 11,000 tests** and lost **fewer than 100 verdicts**, de-duplicated by `(start count, failure set)` — **28 runs, 2026-08-09 18:37 → 2026-08-13 12:41**, parsed by `gate_baseline.parse_run` itself rather than by a fresh regex. Union: **243 distinct failing tests**.

- **LEAVE-ONE-OUT: the union of 27 runs covers the 28th run's failures at a median of 100 % and a mean of 97.3 %.** Eighteen of the 28 are covered *completely*. The three worst — 72.9 %, 79.8 %, 84.3 % — are each a run carrying its own branch's breakage: 14 of one run's 26 residual names are `SkipOrchestrator*`, 20 of another's are a mutation control's intended kills, 12 of a third's are schema-migration tests. **The residual is the diff.** That is the gate's NEW column working, not the population moving.
- **The union has stopped growing.** Per-run additions in time order: `+79 +32 +5 +3 +8 +2 +30 +9 +13 +4 +3 +1 +1 +0 +3 +0 +0 +35 +10 +1 +0 +0 +2 +0 +0 +1 +1 +0`. Eight of the last eleven runs added zero or one name, and both late spikes (+30, +35) are branch-breakage runs above.
- **155 names have been seen in ≥3 of the 28 runs, and they alone cover a mean 96.0 % of any single run's failures** (172 names at ≥2 cover 97.3 %). The other 71 are singletons, mostly branch-specific. *That* is the size of the thing, and it is a name list.
- **The FILE is behind the population, not chasing a moving one.** All **121** committed entries appear in these 28 logs — zero are stale — but the file is a 10-observation sample and is missing **45 of the recurring 155**. So expect `--accept-baseline` to keep adding names for several more accepts. That is the file catching up to a bounded set, and it converges; it is not evidence of a tail nobody can bound.

**The failing set still ROTATES, and that half of the record is right — but rotation is CHURN INSIDE A BOUNDED SET, and the two were conflated.** Pairwise Jaccard over all **378 pairs** of the 28 runs: median **0.515**, mean 0.513, range 0.253–0.798 — consistent with the 0.46 measured on two runs on 2026-08-01. So *which* half of the known population fails tonight is close to a coin flip, which is exactly why a flat exact set is unusable and why the `seen`/`failed` tiering exists. It is **not** a reason a name list cannot bound the population: the same data says a 243-name list leaves a typical run with **zero** unabsorbed failures. Do not quote "the failing set rotates" as an argument that accumulation cannot converge — measured, it converges.
...
**Which suites, ranked rather than sampled (o89d R2, 28 full-plan runs, mean failures per run / runs present in):** `BackgroundGrantBudget` 10.9 / 28-of-28 · `SilenceCompressionCoordinator` 9.1 / 26 · `BackgroundProcessingService` 8.1 / 25 · `PlaybackInterruption` 4.4 / 25 · `ForceQuitResume` 4.1 / 28 · `SpeechRecognitionRequestGate` 3.4 / 28 · `BackgroundProcessingServiceLedger` 3.3 / 25 · `RouteChange` 2.9 / 27 · `BackgroundProcessingServiceTelemetry` 2.7 / 27 · `AnalysisPipelineStallInvariantHarness` 2.7 / 26 · `InterruptionHandling` 1.9 / 26. **Two earlier top-6 lists — 2026-07-29's and 2026-08-13's — were read as evidence the population had MOVED ("near-disjoint, not one name in common"), and it had not.** They are two truncations of one stable ranking: the 07-29 list omitted the single largest contributor and the 08-13 list omitted the second and third, each of which is present in 25–26 of the 28 runs. Rank the whole thing before concluding anything drifted.
