...
**THE UNION DOES NOT SATURATE, and the `~47` above is well under what the file has already recorded (measured 2026-08-13, playhead-o89d; corrected at review).** "Once the union saturates, `RED (N known / 0 new)` is the steady state" was the plan and the data refuses it. Cumulative union size per accepted run, reconstructed from `runs_observed − seen_runs + 1`: **32, 41, 69, 73, 73, 81, 81, 83, 117** — i.e. **+32, +9, +28, +4, +0, +8, +0, +2, +34** — and folding in the **four** full-plan logs of 2026-08-13 (10:23, 11:09, 11:47 and this bead's own 12:41 merge gate) adds **+10, +17, +9, +6** for a union of **146**. There is no knee, but there is also no steady rate: the per-run increment swings between **0 and 34**, so quote the range, not an average. `~47` came from capture–recapture on two runs, and two runs cannot see a tail that is half singletons. `RED (N known / several new)` — not `/ 0 new` — is the honest steady state until something moves the population itself.

**DO NOT QUOTE A SPECIES-RICHNESS ESTIMATE OFF THIS FILE — it is not stable enough to carry a number (measured at o89d review).** The first version of this paragraph said Chao1 gives "a lower bound of **377**", and it did: on the 9-observation / 117-entry file, f1 = 51 and f2 = 5, so `117 + 51²/10 = 377`. **The very next commit in the same series invalidated it.** One accepted run moved 21 names from f1 into f2 (f1 51 → 30, f2 5 → 30), and the same estimator on the file committed here reads **136.0**. An independent Chao2 over the four same-day full-plan runs above reads **176**. Three estimates — 377, 176, 136 — from the same population inside one day, because `f1²/(2·f2)` explodes when f2 is small and this record's f2 was 5. Two further traps the original wording fell into, both the standing defect class: 377 was computed with `S_obs = 117` and then compared against "136 ever observed" (with `S_obs = 136` the same spectrum gives 396), so the "~35 % sample" mixed a numerator and denominator from different populations; and "390 tests carry a `.timeLimit(.minutes(1))`, which is where 377 lands" is a coincidence of two unrelated counts, not corroboration. **The claim that survives all three estimates is the qualitative one, and it is the only one worth acting on: every estimate exceeds what any single run observes, and the observed union has grown on every run so far.**
