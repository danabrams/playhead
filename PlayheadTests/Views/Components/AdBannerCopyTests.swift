...
final class AdBannerQueueTests: XCTestCase {
    ...
    private func makeBannerItem(
        ...
        adStartTime: Double = 120.0,
        adEndTime: Double = 180.0
    ) -> AdSkipBannerItem {
        AdSkipBannerItem(
            id: id,
            windowId: "w-\(id)",
    ...
    func testEnqueueShowsFirstBannerImmediately() {
        let queue = AdBannerQueue()
        let item = makeBannerItem(id: "first")
    ...
    func testDismissClearsCurrentBanner() {
        let queue = AdBannerQueue()
        queue.enqueue(makeBannerItem(id: "dismiss-test"))
    ...
    func testAdjacentSkipsCoalesce() {
        let queue = AdBannerQueue()
        // First skip: 120-180
        queue.enqueue(makeBannerItem(id: "a", adStartTime: 120, adEndTime: 180))
        // Second skip starts within coalesce gap (10s) of first end: 185-240
        queue.enqueue(makeBannerItem(id: "b", adStartTime: 185, adEndTime: 240))

        // Should coalesce — only one banner shown, updated to the second item
        XCTAssertEqual(queue.currentBanner?.id, "b",
            "Adjacent skips within coalesce gap should merge into one banner")
    }

    func testNonAdjacentSkipsQueue() {
        let queue = AdBannerQueue()
        // First skip: 120-180
        queue.enqueue(makeBannerItem(id: "a", adStartTime: 120, adEndTime: 180))
        // Second skip far away: 300-360 (120s gap, well beyond 10s coalesce threshold)
        queue.enqueue(makeBannerItem(id: "b", adStartTime: 300, adEndTime: 360))

        // First banner shows
        XCTAssertEqual(queue.currentBanner?.id, "a")

        // Dismiss first, second should appear (after animation delay)
        queue.dismiss()
        // Note: the queue uses a Task.sleep for animation delay,
        // so the second banner won't appear synchronously in this test.
        // We verify the first was dismissed correctly.
        XCTAssertNil(queue.currentBanner)
    }
}
