...
struct StingerBankTests {
    ...
    func bundledBankPins() throws {
        ...
        // artifact earaudit-oracle-gold-c45c86…3565.json (full-corpus,
        // 16 kHz; regenerated for playhead-xsdz.38 to add
        // gridMaxPodMultiple — template bytes unchanged from v3). If
        // someone regenerates it, these pins document exactly what shipped
        // and force the change through review.
        ...
        // playhead-xsdz.38: morbid's learned max pod multiple is 3 (pods
        // ≤ 90s — the emitter's observed on-grid maximum, matching Dan's
        // ear audits). No grid ⇒ no cap for nikki.
        #expect(bank.entry(forShowKey: "morbid")?.gridMaxPodMultiple == 3)
        #expect(bank.entry(forShowKey: "the-nikki-glaser-podcast")?.gridMaxPodMultiple == nil)
