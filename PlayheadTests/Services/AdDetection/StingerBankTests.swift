...
struct StingerBankTests {
    ...
    func validPayloadDecodes() throws {
        ...
        // playhead-xsdz.38: the learned max pod multiple rides beside the
        // grid and decodes as an Int.
        #expect(entry.gridMaxPodMultiple == 3)
    ...
    @Test("gridMaxPodMultiple below 1 is rejected loudly (playhead-xsdz.38)")
    func nonPositiveGridMaxPodMultipleRejected() throws {
        Self.expectMalformed(
            try Self.mutatedShow { $0["gridMaxPodMultiple"] = 0 },
            containing: "gridMaxPodMultiple 0"
        )
    }

    @Test("gridMaxPodMultiple without a grid is rejected loudly (playhead-xsdz.38)")
    func gridMaxPodMultipleWithoutGridRejected() throws {
        // The emitter only ever writes the cap beside a grid; a cap alone
        // would silently do nothing at refine time — reject it loudly so a
        // hand-edited bank fails fast.
        Self.expectMalformed(
            try Self.mutatedShow { $0.removeValue(forKey: "podWidthGridSeconds") },
            containing: "gridMaxPodMultiple without podWidthGridSeconds"
        )
    }

    @Test("A grid without gridMaxPodMultiple stays valid (legacy uncapped banks)")
    func gridWithoutMaxMultipleDecodes() throws {
        let bank = try StingerBank.decode(
            try Self.mutatedShow { $0.removeValue(forKey: "gridMaxPodMultiple") }
        )
        let entry = try #require(bank.entry(forShowKey: "test-show"))
        #expect(entry.podWidthGridSeconds == 30.0)
    ...
    func bundledBankPins() throws {
        ...
        // artifact earaudit-oracle-gold-836b81…f220.json (gold v6,
        // full-corpus, 16 kHz; playhead-xsdz.39). If someone regenerates
        // it, these pins document exactly what shipped and force the change
        // through review.
        ...
        for show in bank.shows {
            ...
            let slug = show.showKeys[0]
            // Full-width templates (7s × 50 Hz), post edge at frame 50.
            // Every shipped show has a post side.
            let post = try #require(show.post, "\(show.showName): post (return-sting) side expected")
            #expect(post.edgeSampleIndex == 50)
            #expect(post.template.count == 350, "\(show.showName) post: template must be 350 frames")
            #expect(post.confidence > 0 && post.confidence <= 1)
            #expect(post.support >= 2)
            #expect(abs(post.edgeOffsetSeconds) < 5, "\(show.showName) post: learned offset out of sane range")
            if postOnly.contains(slug) {
                #expect(show.pre == nil, "\(show.showName): ships post-only (return-sting)")
            } else {
                #expect(bothSides.contains(slug), "\(show.showName): unexpected show in bank")
                // pre edge at frame 300 (the offline TEMPLATE_INNER/OUTER geometry).
                let pre = try #require(show.pre, "\(show.showName): pre side expected")
                #expect(pre.edgeSampleIndex == 300)
                #expect(pre.template.count == 350, "\(show.showName) pre: template must be 350 frames")
                #expect(pre.confidence > 0 && pre.confidence <= 1)
                #expect(pre.support >= 2)
                #expect(abs(pre.edgeOffsetSeconds) < 5, "\(show.showName) pre: learned offset out of sane range")
            }
        }

        // Grid values: only morbid earned the 30s pod grid (max multiple 3,
        // pods ≤ 90s — the emitter's observed on-grid maximum, matching
        // Dan's ear audits). Every other show is below the 0.6 on-grid
