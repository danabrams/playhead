// SwiftDataNewEpisodeAnnouncerSourceCanaryTests.swift
//
...
// `SwiftDataNewEpisodeAnnouncer.resolveCandidates(keys:context:)`.
//
...
// (Activity widget for the original case; the new-episode notification
// path here). The cycle-8 M3 propagation comment in
// `SwiftDataNewEpisodeAnnouncer.swift:79` records the rationale; this
// canary turns that rationale into a build-time regression net mirror-
// matching the existing `ActivitySnapshotProviderSourceCanaryTests`.
//
...
final class SwiftDataNewEpisodeAnnouncerSourceCanaryTests: XCTestCase {

    /// Cycle-18 L-5: the `keys` parameter on
    /// `resolveCandidates(keys:context:)` MUST be typed as `[String]`
    /// (Array), and the `#Predicate` body MUST consume that local via
    /// `keys.contains($0.canonicalEpisodeKey)`. We assert three things:
    ...
    func testResolveCandidatesKeysParameterStaysArrayTyped() throws {
        let source = try SwiftSourceInspector.loadSource(
            repoRelativePath: "Playhead/Services/Notifications/SwiftDataNewEpisodeAnnouncer.swift"
        )
        ...
        XCTAssertNotNil(
            ...
            """
            `SwiftDataNewEpisodeAnnouncer.resolveCandidates(...)` no \
            longer declares `keys: [String]` (Array typing). Cycle-7 H1 \
            / cycle-8 M3: SwiftData's `#Predicate` translator handles \
            `Array.contains` reliably across iOS / macOS Catalyst builds, \
            but `Set.contains` can silently fail to translate, emptying \
            the new-episode notification path. Restore Array typing OR \
            update this canary if the predicate-input shape intentionally \
            moved (and the new shape has been validated against the \
            same toolchain matrix).
            """
        )
        ...
        XCTAssertNil(
            ...
            """
            `SwiftDataNewEpisodeAnnouncer.resolveCandidates(...)` re- \
            introduced `keys: Set<String>` (whitespace-tolerant match). \
            Cycle-8 M3 documented why this is unsafe: SwiftData's \
            `#Predicate` translation against a Swift `Set` is fragile \
            across toolchain revs and silently empties the new-episode \
            notification path on the bad combinations. Use \
            `keys: [String]` instead.
            """
        )
        ...
        XCTAssertNotNil(
            ...
            """
            `SwiftDataNewEpisodeAnnouncer.resolveCandidates(...)` no \
            longer references `keys.contains($0.canonicalEpisodeKey)` \
            in its `#Predicate` (whitespace-tolerant match). The cycle-8 \
            M3 fix relies on the predicate consuming the Array-typed \
            local so SwiftData's translator takes the Array.contains \
            branch. Restore the predicate OR update this canary if the \
            predicate intentionally moved.
            """
        )
    }
