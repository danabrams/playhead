This is the SECOND round of review for this plan. You reviewed the first version and gave a substantial set of revisions, almost all of which were integrated. Please now stress-test the revised plan and design — not for "what's missing" (most prior gaps are addressed) but for "what could break under contact with reality."

For each proposed change, give your detailed analysis and rationale/justification along with the git-diff style change versus the version shown below.

CONTEXT FOR YOUR ROUND-2 REVIEW (please weight these as you reason):

- This is an iOS podcast app (Playhead) whose differentiated value is on-device ad detection / skipping. All transcription, classification, and decision-making must remain on device for legal reasons. Server-assisted detection is out of scope.
- The codebase is mature through Phase 7 / ef2.6.3 (see §1 of the plan). Please do NOT propose rebuilding shipped components; focus on the gap list and the new contracts introduced in this revision.
- Deployment target is iOS 26+ (FoundationModels API). Apple Intelligence eligibility (15 Pro / 16 / 17 family + region + language + user-toggle + current model availability) is now modeled as a first-class `AnalysisEligibility` contract.

WHAT CHANGED SINCE ROUND 1 (so you can avoid re-suggesting wins):

1. **`AnalysisEligibility` contract added** as Phase 0 deliverable; cohort-split gates (transport vs analysis) replace the single device-set gate.
2. **Single Readiness SLO replaced with 5 cohort SLIs**: `time_to_downloaded`, `time_to_proximal_skip_ready`, `false_ready_rate`, `unattributed_pause_rate`, `warm_resume_hit_rate`. Plus an executive north-star seed retained for capacity-planning conversations only.
3. **Three-layer cause taxonomy**: `InternalMissCause` (15 elements, scheduler-facing) → `SurfaceReason` (8 elements, copy-stable, user-facing) → `ResolutionHint`. Some internal causes deliberately do NOT map to user-visible Paused.
4. **Phase 1.5 inserted** between "executes reliably" (Phase 1) and "ship public surfaces" (Phase 2). Introduces `EpisodeSurfaceStatus` as the sole reducer from persisted facts to UI; impossible-state assertions; 10 days of dogfood; `false_ready_rate` gate.
5. **Skip-ready badge retightened**: `playbackReadiness ∈ { none, deferredOnly, proximal, complete }`. Library cells show ✓ only for `proximal` or `complete`. Added new "Deferred-only" and "Analysis unavailable" states in §5.
6. **Three internal storage classes** (`media` / `warmResumeBundle` / `scratch`) replace the two-tier framing.
7. **`EpisodeExecutionLease` + append-only `WorkJournal`** — single-writer-per-episode contract across all four execution planes (URLSession, BGProcessingTask, BGContinuedProcessingTask, foreground assist).
8. **Phase 1 acceptance is now three independent gates** (Transport / Checkpoint / First-window readiness) against a 12-episode fixture corpus across 30 device-nights.
9. **B5 fully specified** as a bounded local controller: EWMA/Beta estimators per cohort, defined actuator set, hard safety rails (max one-notch per actuator per 24h, never zero out an episode).
10. **Notifications** kept minimal but split into two narrow opt-in classes: trip-readiness + action-required (only fires when entire batch blocked by a *user-fixable* `SurfaceReason`).
11. **6 new risk rows** including force-quit-blocks-relaunch, lease/journal corruption, `warmResumeBundle` version skew, diagnostics leakage.
12. **UI design doc** (Appendix A) updated to match: new states (Deferred-only, Analysis unavailable), Live Activity carve-out, two notification classes, "All unplayed" removed from stepper.

WHAT I MOST WANT YOU TO STRESS-TEST IN ROUND 2:

A. **The cohort SLI set.** Are these the right 5? Is one missing (e.g., a *user-perceived* metric like "time from Download tap to first auto-skipped ad on play")? Are the cohort dimensions (download type × eligibility × network/power × fixture type) the right cuts, or are we slicing too thin to keep statistical power? What thresholds would you actually defend for each SLI, given on-device-only measurement constraints?

B. **The `InternalMissCause` → `SurfaceReason` mapping.** Walk the 15-element internal enum and try to break it. Are there any internal causes that don't cleanly map to exactly one surface reason, or that should sometimes map differently depending on context (e.g., does `model_unavailable` mean "Analysis unavailable" permanently or "Couldn't analyze" transiently)? Is the 8-element surface enum collapsible to 5–6, or do we need 10?

C. **`EpisodeExecutionLease` cross-episode interactions.** The lease is single-writer-per-episode. But scheduling decisions inherently cross episodes (e.g., "promote episode X to Now lane and demote Y"). Is there a missing `SchedulerEpoch` or `SchedulingTransaction` primitive that should sit above the per-episode lease? What are the failure modes when episode-level leases conflict with global scheduler decisions?

D. **`playbackReadiness` enum cardinality.** Four states: `none`, `deferredOnly`, `proximal`, `complete`. Is there a missing fifth state for "proximal-but-uncertain" (e.g., we believe the next 18 minutes are covered but FM confidence on one of the spans is borderline)? Or is forcing every span into the binary auto-skip-vs-markOnly buckets (per ef2.6.3) sufficient?

E. **The B5 controller's cold-start problem.** EWMA/Beta estimators need observations. On first install, every cohort has zero observations. What does the controller do? Static defaults from a per-device-class config table (Phase 1) handed off to the controller (Phase 3)? Some kind of pretrained prior shipped with the app (legal implications)? How does this transition handoff actually work?

F. **The fixture corpus selection bias.** 12 fixed episodes hand-picked by us could systematically miss real-world failure modes. Should we instead specify a *taxonomy* the corpus must cover (e.g., {chapter-richness} × {ad-density} × {language} × {duration-bucket}) and require each Phase 1 gate to randomly sample from each cell? What's the right balance between reproducibility (fixed corpus) and external validity (random sampling)?

G. **Phase 1.5's `EpisodeSurfaceStatus` reducer as the sole UI input.** This is a strong constraint that will be tested every time someone adds a UI surface (widget, Live Activity, share sheet, App Intents shortcut, watchOS extension if it ever ships). Is this constraint actually enforceable, or will the team work around it under deadline pressure? What enforcement mechanism survives that?

H. **`warm_resume_hit_rate` as an SLI is unusual** — it measures effectiveness of an internal optimization, not a user-perceived outcome. Is it actually a load-bearing metric, or is it a vanity metric that incentivizes the team to over-retain?

I. **Action-required notifications creep.** I gave you my caveat that this class could grow beyond its constrained envelope. Is there a structural way to *prevent* it from growing (e.g., a permission scope that literally can't fire for transient causes), or is the constraint policy-only?

J. **Anything you'd actually delete.** Is there scope in this plan that should be cut entirely? A phase that should merge with another? A risk that's overstated? An "honesty principle" that's actually an over-correction? Be willing to argue for *less*.

================================================================================
MAIN PLAN — podcast_background_processing_plan.md
================================================================================
...
*Prepared for planning • April 16, 2026 • Revision 2 (post-UI-brainstorm and codebase reconciliation)*
...
| **Per-show capability profiles** | ❌ | Phase 2 |
...
**Cross-cutting concurrency contract (NEW):** All four execution planes above can touch the same episode. Add an `EpisodeExecutionLease` + append-only `WorkJournal` between transport and analysis:
...
Without this, the failure mode is silent and ugly: duplicate analysis, checkpoint corruption, badge drift, or retained-bundle skew. High blast radius, low likelihood of being noticed early.
...
3. **Replace the single Readiness SLO with cohort-based SLIs.** Operational measurement layer:
   ...
   - `false_ready_rate` *(the honesty SLI — fraction of cells showing ✓ where playback didn't actually skip in the next listening window)*
   - `unattributed_pause_rate`
   - `warm_resume_hit_rate` *(B4 effectiveness — fraction of re-downloads that became Skip-ready instantly from the retained `warmResumeBundle`)*

   Measure by cohort:
   - explicit Download vs subscription auto-download
   - analysis-eligible vs transport-only device
   - Wi-Fi + power vs constrained conditions
   - chapter-rich / chapter-poor / no-ad fixture types
...
4. **Define cause taxonomy as three layers** (instead of a single miss-reason enum):
   - **`InternalMissCause`** (high-cardinality, single-cause, scheduler-facing):
     `{ no_runtime_grant, task_expired, thermal, low_power_mode, battery_low_unplugged, no_network, wifi_required, media_cap, analysis_cap, user_preempted, user_cancelled, model_unavailable, unsupported_language_or_region, asr_failed, pipeline_error }`
   - **`SurfaceReason`** (low-cardinality, copy-stable, user-facing):
     `{ waiting_for_time, phone_is_hot, needs_charger, waiting_for_network, storage_full, analysis_unavailable, cancelled, couldnt_analyze }`
   - **`ResolutionHint`** (what unblocks each surface reason)

   Not every internal cause maps to `Paused`: `no_runtime_grant` is "waiting" (Up Next), `classifier_uncertain` is "still analyzing" or "mark-only fallback," `unsupported_language_or_region` is "Analysis unavailable" (its own state per §5).
...
6. **Define storage budgets and three internal artifact classes** (per §3 retention policy):
   ...
   - Add `warmResumeHitRate` to the SLI set above.
...
**Deliverables:**
1. **Build a fixed device-lab fixture corpus.** 12 episodes spanning chapter-rich, no-chapter, ad-rich, no-ads, short, long, dynamic-inserted, and re-download cases. This corpus is the substrate for every Phase 1 acceptance gate — never ad-hoc episodes.
...
**Deliverables:**
1. Introduce **`EpisodeSurfaceStatus`** as the sole reducer from persisted facts (state machine + `playbackReadiness` + active `InternalMissCause` + `AnalysisEligibility`) to the UI state shown in Library / Detail / Activity. The UI never reads underlying facts directly.
...
5. **State-transition audit + impossible-state assertions.** Compile-time-enforced exhaustive switches over the state enum; runtime assertions that block illegal combinations (e.g., `playbackReadiness == proximal` AND `analysisUnavailableReason != nil`).
...
2. **`playbackReadiness` field on `AnalysisSummary`.** States: `{ none, deferredOnly, proximal, complete }` plus a `coverageRange` describing which spans are confidently classified. Library cells render ✓ **only** for `proximal` or `complete`. `deferredOnly` stays as ⤓.
...
8. **Trip-readiness + action-required notifications.** Single permission ask gated to non-Generic "Download Next N." Two narrow classes per UI design §G: trip-readiness fires when all N hit Downloaded; action-required fires only when the entire batch is blocked by a *user-fixable* `SurfaceReason` (`storage_full`, `wifi_required`, `analysis_unavailable`). At most one of each per batch.
9. **Promote RSS / show-notes pre-seeding from shadow to live signal** at the corroboration gate (≤0.15 of fusion budget).
10. **Per-show capability profiles** — observed (not user-set). Internal classification: chapter-rich, host-read-only, music-bed-reliable, etc. Used by scheduler to size budget; exposed only in Settings → Diagnostics. **Always-on minimum per-episode budget** so a profile can never zero out work for a show.
...
6. **B5 closed-loop control — bounded local controller (not online RL, not opaque self-tuning).**
   - **Observations** (locally logged per slice): device class, eligibility state, work type, episode duration bucket, show profile, queue lane, bytes expected / completed, slice target vs actual runtime, `InternalMissCause`, whether the slice increased proximal coverage.
   - **Estimators**: EWMA / Beta success models per cohort (cohort = device class × work type × show profile).
...
7. **Smart contextual surfacing of "Download Next N"** (Q3 option d) — calendar event detection, charger plug-in, low-connectivity prediction. Gated; opt-out in Settings.
...
1. **`playbackReadiness == proximal` definition.** What is the *exact* coverage threshold (e.g., "all auto-skip-confidence ad spans inside next-listening-window are bounded")? Pin as a versioned policy parameter so the badge promise is stable across model updates.
2. **Cause taxonomy validation.** §6 Phase 0 proposes 15 `InternalMissCause`s and 8 `SurfaceReason`s. Validate against `AnalysisWorkScheduler` realities during Phase 0 implementation; revise if the scheduler can't cleanly attribute each pause to exactly one internal cause.
3. **Per-show capability profile schema.** What fields, when persisted, when surfaced. Affects Diagnostics surface and the "this show isn't worth analyzing" modulation (capped by always-on minimum from Phase 2 deliverable 10).
4. **Cohort SLI thresholds.** §6 Phase 0 introduces 5 cohort SLIs; Phase 1.5 needs concrete `false_ready_rate` threshold (suggested seed ≤2%); other SLIs need calibration after Phase 0 measurement. Numbers are not commitments yet.
...
| `warmResumeBundle` retention accumulates without bound | L | M | Analysis cap (default 200 MB) enforces upper bound; LRU eviction within the cap; `warm_resume_hit_rate` SLI tracks effectiveness. |
...
| Per-show capability profiles encode bias (e.g., "this show is hopeless") and starve worthwhile analysis | M | M | Always-on minimum analysis budget per episode (Phase 2 deliverable 10); capability profiles only modulate, never zero out. |
...
| UI surfaces evolve faster than `EpisodeSurfaceStatus` reducer; UI starts reading underlying facts directly | M | M | Phase 1.5 deliverable 1 makes the reducer the *sole* path; impossible-state assertions catch direct reads in dogfood. |
...
================================================================================
APPENDIX A — UI DESIGN DOC (docs/plans/2026-04-16-podcast-bg-ui-design.md)
================================================================================
...
### Header — ambient gauge

A single compact summary strip at the top:

> **Today: 12 downloaded · 11 skip-ready · 1 paused (storage)**

This mirrors the readiness SLO without exposing the SLO concept by name. Tap → expands into a 7-day mini-history sparkline (v2 only).
...
- **Analysis-data cap:** 50 MB / 200 MB / 1 GB / Unlimited. Default 200 MB. Sub-line: "Holds analysis for thousands of episodes."
