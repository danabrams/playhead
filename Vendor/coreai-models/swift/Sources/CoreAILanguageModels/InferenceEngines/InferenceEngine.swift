...
// Playhead vendored-fork guard (playhead-b6jq PR 1): `EmbeddedInput`
// stores a CoreAI `NDArray`, and the CoreAI framework ships only in
// device SDKs. Compile the multimodal protocol out with it so the
// package builds for the iOS simulator. Device builds are unchanged.
#if canImport(CoreAI)
